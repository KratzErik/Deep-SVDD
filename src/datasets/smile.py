...
#        self.out_frac = Cfg.out_frac
    ...
    def build_architecture(self, nnet):
        ...
        if Cfg.architecture not in (1,2,3):
        ...
        elif Cfg.architecture == 1: # For 256by256 images
            ...
            if Cfg.weight_dict_init & (not nnet.pretrained):
                # initialize first layer filters by atoms of a dictionary
                W1_init = learn_dictionary(nnet.data._X_train, n_filters=8, filter_size=5, n_sample=Cfg.n_dict_learn)
                plot_mosaic(W1_init, title="First layer filters initialization",
                            canvas="black",
                            export_pdf=(Cfg.xp_path + "/filters_init"))
            else:
            ...
            if Cfg.dropout:
                nnet.addDropoutLayer()
            if Cfg.bias:
                nnet.addDenseLayer(num_units=Cfg.rep_dim * units_multiplier)
            else:
                nnet.addDenseLayer(num_units=Cfg.rep_dim * units_multiplier,
                                   b=None)

        elif Cfg.architecture == 3:
            
            #(192,320) input: (2,2) maxpooling down to (3,5)-image before dense layer

            if Cfg.weight_dict_init & (not nnet.pretrained):
                # initialize first layer filters by atoms of a dictionary
                W1_init = learn_dictionary(nnet.data._X_train, n_filters=16, filter_size=5, n_sample=Cfg.n_dict_learn)
                plot_mosaic(W1_init, title="First layer filters initialization",
                            canvas="black",
                            export_pdf=(Cfg.xp_path + "/filters_init"))
            else:
            ...
            # dense layer 1
            if Cfg.bias:
                nnet.addDenseLayer(num_units=Cfg.rep_dim)
            else:
                nnet.addDenseLayer(num_units=Cfg.rep_dim, b=None)


        elif Cfg.architecture == 4:
    ...
    def build_autoencoder(self, nnet):
        ...
        if Cfg.architecture == 1:
            first_layer_n_filters = 16
            if Cfg.weight_dict_init & (not nnet.pretrained):
                # initialize first layer filters by atoms of a dictionary
                W1_init = learn_dictionary(nnet.data._X_train, first_layer_n_filters, 5, n_sample=Cfg.n_dict_learn)
                plot_mosaic(W1_init, title="First layer filters initialization",
                            canvas="black",
                            export_pdf=(Cfg.xp_path + "/filters_init"))
            else:
            ...
            if Cfg.bias:
                ...
                nnet.addDenseLayer(num_units=Cfg.rep_dim, b=None)
            nnet.setFeatureLayer()  # set the currently highest layer to be the SVDD feature layer
            nnet.addReshapeLayer(shape=([0], (Cfg.rep_dim / 16), 4, 4))
            if Cfg.leaky_relu:
                nnet.addLeakyReLU()
            else:
                nnet.addReLU()
        ...
        #(192,320) input: (2,2) maxpooling down to (3,5)-image before dense layer
            ...
            nnet.addDenseLayer(num_units=Cfg.rep_dim, b=None)
            nnet.setFeatureLayer()  # set the currently highest layer to be the SVDD feature layer
            nnet.addReshapeLayer(shape=([0], (Cfg.rep_dim / (3*5)), 3, 5))
            if Cfg.leaky_relu:
                nnet.addLeakyReLU()
            else:
                nnet.addReLU()
        ...
        if Cfg.architecture == 4:

            # input (256,256)
            if Cfg.weight_dict_init & (not nnet.pretrained):
                # initialize first layer filters by atoms of a dictionary
                W1_init = learn_dictionary(nnet.data._X_train, 16, 5, n_sample=Cfg.n_dict_learn)
                plot_mosaic(W1_init, title="First layer filters initialization",
                            canvas="black",
                            export_pdf=(Cfg.xp_path + "/filters_init"))
            ...
            nnet.addDenseLayer(num_units=Cfg.rep_dim, b=None)
            nnet.setFeatureLayer()  # set the currently highest layer to be the SVDD feature layer
            nnet.addReshapeLayer(shape=([0], (Cfg.rep_dim / (2*2)), 2, 2))
            if Cfg.leaky_relu:
                nnet.addLeakyReLU()
            else:
                nnet.addReLU()
